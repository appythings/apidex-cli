...
### Added
- `upload-spec` and `validate` accept MCP tools-catalogue specs (`portalType: mcp`, `spec:` file pointer) and GraphQL v1 upload envelopes (`portalType: graphql`, product-only). `openapi:` remains the REST alias. Overlays stay OpenAPI-only.
